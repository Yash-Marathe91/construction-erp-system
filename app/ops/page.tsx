...
export default function OpsPage({
  ...
  const [workers, setWorkers] = useState<any[]>([
    { id: 'loading', name: 'Loading...', role: '...', status: '...', checkIn: '...' }
  ]);
    ...
    async function fetchWorkers() {
      ...
      if (dbWorkers) {
        setWorkers(dbWorkers.map((w: any) => {
          // Filter attendance to just today in JS if the query didn't perfectly filter it
          ...
          return {
            ...
            name: w.name,
            role: w.role,
            daily_wage: w.daily_wage, // Needed for payroll
                        ...
                        <div className="text-right">
                          {worker.status === 'Present' ? (
                            ...
                            <button 
                              ...
                              className="flex items-center gap-1 text-[10px] font-bold text-[#ba1a1a] bg-[#ffdad6] px-2 py-0.5 rounded-full hover:bg-green-100 hover:text-green-800 transition-colors cursor-pointer"
      ...
      <AttendanceSheetModal 
         ...
         onAttendanceMarked={(workerId, status) => {
           setWorkers(prev => prev.map(w => w.id === workerId ? { 
             ...w, 
             status: status, 
             checkIn: status === 'Present' ? new Date().toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' }) : '-' 
           } : w));
         }}
      />
    </AppLayout>
