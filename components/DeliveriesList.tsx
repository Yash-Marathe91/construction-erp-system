        ...
        {deliveries.map((delivery) => (
          ...
          >
            <CardContent className="p-0 flex">
               <div className={`w-1.5 ${delivery.status === 'In Transit' ? 'bg-[#feda5a]' : 'bg-[#e7eeff]'}`} />
               <div className="flex-1 p-4">
                  <div className="flex justify-between items-start">
                    <div className="flex gap-4 items-center">
                      ...
                      <div>
                        <h3 className="font-heading font-bold text-[#182232] text-lg">{delivery.name}</h3>
                  ...
                  <div className="mt-5 flex items-end justify-between border-t border-gray-50 pt-3">
                    <div>
                      ...
                      <p className="font-heading font-bold text-[#182232] text-sm">{delivery.expected}</p>
                    </div>
                    <span className={`text-[10px] font-bold px-3 py-1 flex items-center gap-2 rounded-full shadow-sm ${
                      delivery.status === 'In Transit' ? 'bg-[#feda5a] text-[#182232]' : 'bg-[#f0f3ff] text-[#182232]'
                    }`}>
                       <div className={`w-1.5 h-1.5 rounded-full ${delivery.status === 'In Transit' ? 'bg-[#182232] animate-pulse' : 'bg-[#182232]'}`} />
                       {delivery.status}
                    </span>
                  </div>
               </div>
